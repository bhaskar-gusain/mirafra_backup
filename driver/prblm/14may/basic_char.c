		...
		loff_t *offset)
{
	...
	int out_len = snprintf(out,BUF_SIZE,"Result - %d\n",result);
		...
		loff_t *offset)
{
	...
	/*
	for(int i = 0 , j = buffer_size - 1 ; i < j ; i++ , j--)
	{
	...
	*/
	...
	if (sscanf(kernel_buffer, "%d %d %c", &num1, &num2, &op) != 3)
	{
	...
	printk(KERN_INFO "calc: completed %d %c %d = %d\n", num1, op, num2, result);
